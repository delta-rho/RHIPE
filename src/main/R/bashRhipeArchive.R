...
bashRhipeArchive <- function(archive.base.name = "RhipeLib", delete.local.tmp.dir = TRUE) {
   ...
   file.copy <- function(src, dest, recursive = TRUE, ...) {
      if (recursive) 
         system(paste("cp -RLf", src, dest)) else system(paste("cp -Lf", src, dest))
   }
   ...
   ############################################################################
   # Copies the contents of .libPaths() to a destination folder that 
   # must already exist
   ############################################################################
   copy.libPaths <- function(dest) {
      ...
      for (p in lib.paths) {
         ...
         if (length(contents) == 0) 
            next
         for (d in contents) file.copy(d, dest, recursive = TRUE)
      }
   }

   ############################################################################
   # Find exectuables and shared Libs recursively Two types of strats: 1) find
   # executables using a test I found on the internet 2) find things with .so
   # extension.  Works off current working directory.
   ############################################################################
   findExecutables <- function() {
      ...
      shared.libs <- list.files(pattern = paste(.Platform$dynlib.ext, "$", sep = ""), 
   ...
   ############################################################################
   # sharedLibs Finds shared libs that an exec depends on with 'ldd' Designed 
   # for multiple files.  Does not insert the same shared lib more then once.
   ############################################################################
   sharedLibs <- function(files) {
      ...
      for (f in files) {
         try({
            ...
            b <- lapply(strsplit(a, "=>"), function(r) if (length(r) >= 2) 
              r[2] else NULL)
            b <- strsplit(unlist(b), " ")
            b <- unlist(lapply(b, "[[", 2))
            for (lib in b) if (nchar(lib) > 0) 
              shared[[lib]] <- lib
         }, silent = TRUE)
      }
   ...
   if (delete.local.tmp.dir && file.exists(tfolder)) {
      ...
      Sys.chmod(tfolder)
      try({
         unlink(tfolder, recursive = TRUE, force = TRUE)
      }, silent = TRUE)
   }
