...
.rhmr <- function(map = NULL, reduce = NULL, combiner = FALSE, setup = NULL, cleanup = NULL, 
   ...
   if (is.list(cleanup)) {
      ...
      if (is.null(cleanup$reduce)) 
         cleanup$reduce <- expression()
      if (is.null(cleanup$map)) 
         cleanup$map <- expression()
   }
   if (is.null(cleanup)) 
      cleanup <- expression()
   ...
   lines <- append(lines, list(R_HOME = R.home(), rhipe_command = paste(rhoptions()$runner, 
      collapse = " ")))
   ...
   if (rhoptions()$copyObjects$auto) {
      ...
      require(codetools)
      exp <- list(setup$map, setup$reduce, cleanup$map, cleanup$reduce, map, reduce$pre, 
         reduce$post, reduce$reduce)
      calling.frame <- envir
      seen.vars <- new.env()
      old.vars <- new.env()
      getV <- function(mu, cf) {
         ...
         .getV <- function(mu) {
            ...
            varns <- x$variables[!sapply(x$variables, exists, where = omit, inherits = FALSE, 
              USE.NAMES = FALSE)]
            fns <- as.logical(unlist(sapply(varns, function(a) is.function(tryCatch(get(a, 
              cf), error = function(e) NULL)))))
            xfns <- unique(c(x$functions, varns[fns]))
            xfns <- xfns[!sapply(xfns, exists, where = omit, inherits = FALSE, 
              USE.NAMES = FALSE)]
            varns <- varns[!fns]
            varns <- elim(varns, cf)
            xfns <- elim(xfns, cf)
         ...
         res <- .getV(mu)
         return(unique(unlist(c(res$varns, res$funs, sapply(res$varns, function(kap) {
            moz <- tryCatch(get(kap, envir = cf), error = function(e) NULL)
            if (is(moz, "list")) {
              sapply(moz, function(af) {
               if (is(af, "function")) getV(af, environment(af))
              })
            }
      ...
      if (length(ls(seen.vars)) > 0) 
         warning(sprintf("RHIPE(param='auto'): Following variables were discovered but not found: %s", 
   ...
   orderp <- switch(pmatch(orderby, ordert), `1` = {
      ...
      "org.godhuli.rhipe.RHInteger"
   }, `3` = {
      "org.godhuli.rhipe.RHNumeric"
   }, `4` = {
      "org.godhuli.rhipe.RHText"
   }, )
   if (is.null(orderp)) 
      stop(sprintf("Wrong ordering %s: try bytes,integer,numeric,character"))
   ...
   if (!is.null(partitioner) && is.list(partitioner)) {
      ...
      if (length(partitioner$lims) == 1) 
         partitioner$lims <- rep(partitioner$lims, 2)
      lines$rhipe_partitioner_start <- partitioner$lims[1]
      lines$rhipe_partitioner_end <- partitioner$lims[2]
      if (!all(partitioner$lims > 0)) 
         stop("limits must be greater than zero")
      ...
      lines$rhipe_partitioner_class <- if (!is.null(partitioner$class)) 
         partitioner$class else {
         switch(lines$rhipe_partitioner_type, string = {
            "org.godhuli.rhipe.RHPartitionerText"
         }, numeric = {
            "org.godhuli.rhipe.RHPartitionerNumeric"
         }, integer = {
            "org.godhuli.rhipe.RHPartitionerInteger"
         })
      }
   }
   ...
   if (is(input, "numeric") || is(input, "integer")) {
      ...
      if (Rhipe:::dir.contain.mapfolders(ifo)) 
         input <- rhoptions()$ioformats[["map"]](ifo) else input <- rhoptions()$ioformats[["seq"]](ifo)
   }
   ...
   lines$rhipe_setup_reduce <- rawToChar(serialize(lines$rhipe_setup_reduce, NULL, 
      ascii = TRUE))
   lines$rhipe_cleanup_map <- rawToChar(serialize(lines$rhipe_cleanup_map, NULL, 
      ascii = TRUE))
   ...
   if (length(zips) > 0) 
      lines$rhipe_zips <- paste(unlist(local({
...
mkdHDFSTempFolder <- function(dirprefix = rhabsolute.hdfs.path(rhoptions()$HADOOP.TMP.FOLDER), 
   pathsep = NULL, file, unqsalt = NULL) {
   if (is.null(pathsep)) {
      pathsep <- if (grepl("/$", dirprefix)) 
         "" else "/"
   }
   if (is.null(unqsalt)) {
   ...
   sprintf("%s%s%s-%s", dirprefix, pathsep, file, .Call("md5", unqsalt, length(unqsalt), 
      PACKAGE = "Rhipe"))
}
...
getObjects <- function(maxsize = if (is.null(rhoptions()$copyObjects)) 100 * 1024 * 
   1024 else rhoptions()$copyObjects$maxsize, excludeObjects = if (is.null(rhoptions()$copyObjects)) character(0) else rhoptions()$copyObjects$exclude) {
   frames <- if (sys.nframe() == 1) 
      0 else c(0, rev(-(2:sys.nframe())))
   varcol <- list()
   for (i in frames) {
      a <- sys.frame(i)
      for (varname in ls(a)) {
         if (varname %in% excludeObjects) 
            next
         x <- get(varname, envir = a)
         if (object.size(x) <= maxsize) 
            varcol[[varname]] <- x else warning(sprintf("Dropped object: ", varname))
      }
   }
   varcol
}
...
makeOmits <- function() {
   ...
   sapply(as.character(.packages()), function(r) {
      sapply(tryCatch(ls(sprintf("package:%s", r), all.names = TRUE), error = function(e) NULL), 
         function(s) assign(s, NA, m))
   })
