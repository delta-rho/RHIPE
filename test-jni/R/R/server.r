rhinit <-function(errors=TRUE, info=FALSE,path=NULL,cleanup=FALSE,bufsize=as.integer(3*1024*1024),buglevel=0,first=TRUE){
  ...
  Rhipe2:::.rhinit(errors,info,path,cleanup,bufsize,buglevel)
  if(first){
    ## if(buglevel>0) message("Initial call to personal server")
    ## Rhipe2:::.rhinit(errors=TRUE,info=if(buglevel) TRUE else FALSE,path,cleanup,bufsize,buglevel=buglevel)
    rhoptions(mode = Rhipe2:::Mode,mropts=rhmropts.1(),quiet=FALSE) # "experimental"
    ## if(buglevel>0) message("Secondary call to personal server")
    ## Rhipe2:::.rhinit(errors=TRUE,info=if(buglevel) TRUE else FALSE,path,cleanup,bufsize,buglevel=buglevel)
    Sys.sleep(2)
    message("Rhipe2 first run complete")
    return(TRUE)
  }
}
...
.rhinit <- function(errors=FALSE, info=FALSE,path=NULL,cleanup=FALSE,bufsize=as.integer(3*1024*1024),buglevel=0){
  ...
  message("Rhipe2 initialization complete")
...
rhmropts.1 <- function(){
  ## List of files,
  v <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhmropts"))
  v
}
...
rhcp.1 <- function(ifile, ofile) {
  ...
  ## v <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhcp",ifile, ofile))
}

rhmv.1 <- function(ifile, ofile) {
  system(command=paste(paste(Sys.getenv("HADOOP_BIN"),  "hadoop",
           sep=.Platform$file.sep), "fs", "-mv", ifile, ofile, sep=" "))
  ## v <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhmv",ifile, ofile))
}
...
rhls.1 <- function(folder,recurse=FALSE){
  ## List of files,
  v <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhls",folder, if(recurse) 1L else 0L))
...
rhdel.1 <- function(folder){
  x <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhdel",folder))
  x[[1]]=="OK"
}

rhget.1 <- function(src, dest){
  x <- Rhipe2:::send.cmd(rhoptions()$child$handle, list("rhget",src,dest))
}

rhput.1 <- function(src, dest,deletedest=TRUE){
  x <- Rhipe2:::send.cmd(rhoptions()$child$handle, list("rhput",src,dest,as.logical(deletedest)))
}
...
rhread.1 <- function(files,type=c("sequence"),max=-1L,mc=FALSE,asraw=FALSE,size=3000,buffsize=1024*1024,quiet=FALSE,...){
  ...
  max <- as.integer(max)
  p <- if(type=="text"){
    Rhipe2:::hmerge(files, buffsize=as.integer(buffsize),max=max,...)
  }else{
    Rhipe2:::send.cmd(rhoptions()$child$handle, list("sequenceAsBinary", files,max,as.integer(rhoptions()$child$bufsize)),
                          getresponse=0L,
                          continuation = function() Rhipe2:::rbstream(rhoptions()$child$handle,size,mc,asraw,quiet))
  }
  p
}
...
rhgetkey.1 <- function(keys,paths,sequence="",skip=0L,mc=FALSE,size=3000,...){
  ...
  paths <- unlist(paths)
  p <- Rhipe2:::send.cmd(rhoptions()$child$handle, list("rhgetkeys", list(keys,paths,sequence,
                   if(sequence=="") FALSE else TRUE,
                   as.integer(skip)))
           ,getresponse=0L,
           conti = function(){
             return(Rhipe2:::rbstream(rhoptions()$child$handle,size,mc))
           })
  p
}
...
rhwrite.1 <- function(lo,dest,N=NULL){
  ...
  p <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("binaryAsSequence",dest,
...
rhkill.1 <- function(x){
  ...
  result <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhkill", list(id)))
}
...
rhstatus.1 <- function(x,mon.sec=0,autokill=TRUE, showErrors=TRUE,verbose=FALSE){
  ...
  if(mon.sec<=0) {
    return(Rhipe2:::.rhstatus.1(id,autokill,showErrors))
  }else{
    while(TRUE){
...
.rhstatus.1 <- function(id,autokill=FALSE,showErrors=FALSE){
  result <- Rhipe2:::send.cmd(rhoptions()$child$handle, list("rhstatus", list(id, as.integer(showErrors))))[[1]]
...
rhjoin.1 <- function(x,verbose=TRUE){
  ...
  result <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhjoin", list(id,
...
hmerge <- function(inputfiles,buffsize=2*1024*1024,max=-1L,verb=FALSE){
  ...
  x <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhcat",inputfiles,as.integer(buffsize),as.integer(max)),
...
rhstreamsequence.1 <- function(inputfile,type='sequence',batch=1000,quiet=TRUE,...){
  ...
  files <- Rhipe2:::getypes(inputfile,type)
  index <- 1;max.file <- length(files)
  if(!quiet) cat(sprintf("Moved to file %s (%s/%s)\n", files[index],index,max.file))
  x <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhopensequencefile",files[1]),getresponse=1L)
  if(x[[1]]=="OK"){
    return(list(get=function(mc=FALSE){
      quantum <- batch
      ## if (rhoptions()[[".code.in"]]!=calledcode) warning("Server has been restarted, excpect an error")
      p <- Rhipe2:::send.cmd(rhoptions()$child$handle, list("rhgetnextkv", files[index],as.integer(quantum))
                               ,getresponse=0L,
                               conti = function(){
                                 return(Rhipe2:::rbstream(rhoptions()$child$handle,size=quantum,mc=mc,quiet=quiet,...))
                               })
      ...
      while(TRUE){
        ...
        if(!quiet) cat(sprintf("Moved to file %s (%s/%s)\n", files[index],index,max.file))
        x <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhopensequencefile",files[index]),getresponse=1L)
        if(x[[1]]!="OK") error(sprintf("Problem reading next in sequence %s",files[index]))
        p <- append(p,Rhipe2:::send.cmd(rhoptions()$child$handle, list("rhgetnextkv", files[index],as.integer(quantum))
                               ,getresponse=0L,
                               conti = function(){
                                 return(Rhipe2:::rbstream(rhoptions()$child$handle,size=quantum,mc=mc,quiet=quiet,...))
                               }))
      ...
      return(p)
         ...
         ## if (rhoptions()[[".code.in"]]!=calledcode) warning("Server has been restarted, excpect an error")
         x <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhclosesequencefile",files[index],getresponse=1L))
         }))
...
rhbiglm.stream.hdfs.1 <- function(filename,type='sequence',modifier=NULL,batch=100,...){
  ...
  return(function(reset=FALSE){
    if(reset){
      ...
      a <<- Rhipe2::rhstreamsequence(filename,type,batch,...)
      modifier(NULL,TRUE)
    }else{
      dd <- a$get()
