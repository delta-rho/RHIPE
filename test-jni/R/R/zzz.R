...
attr(vvvv,'notes') <- c("Major change: call Rhipe2:::first.run() before starting")
...
onload.2 <- function(libname, pkgname){
  ...
  warning("Rhipe2 requires the HADOOP or HADOOP_BIN environment variable to be present\n $HADOOP/bin/hadoop or $HADOOP_BIN/hadoop should exists")
  ...
  ## Rhipe2:::.rh.first.run()
  message("--------------------------------------------------------
| IMPORTANT: Before using Rhipe2 call rhinit() |
| Rhipe2 will not work or most probably crash            |
--------------------------------------------------------")
}

first.run <- function(buglevel=0){
  ## if(buglevel>0) message("Initial call to personal server")
  ## Rhipe2::rhinit(errors=TRUE,info=if(buglevel) TRUE else FALSE,buglevel=buglevel)
...
##   opts$socket <- rhGetConnection(paste(cp,collapse=":"),opts$port)
  ...
  if(Sys.getenv("HADOOP")=="") stop("Rhipe2 requires the HADOOP environment variable to be present")
  if(.Platform$r_arch!="")
    opts$runner <- list.files(paste(system.file(package="Rhipe2"),"libs",.Platform$r_arch,
                                    sep=.Platform$file.sep),pattern="imperious.so",full=T)
  else
