...
##'org.apache.hadoop.mapred.SequenceFileOutputFormat'
  ...
  lines$rhipe_map_buffer_bytes <- 10
...
##1
    ...
    if(missing(N)){
      aa <- Rhipe2:::optmerge( rhoptions()$mropts,mapred)
...
rhex <- function (conf,async=FALSE,mapred,...) 
{
  ...
  if(rhoptions()$mode=="current"){
    ...
    result <- Rhipe2:::send.cmd(rhoptions()$child$handle,list("rhex", zonf))
...
rhstatus <- function(x){
  ...
  result <- Rhipe2:::doCMD(rhoptions()$cmd['status'],jobid =id,
...
rhjoin <- function(x,verbose=TRUE,ignore.stderr=TRUE){
  ...
  result <- Rhipe2:::doCMD(rhoptions()$cmd['join'], jobid =job.id,needoutput=TRUE,
