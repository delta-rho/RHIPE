...
public class Richmond {
    ...
    public Richmond(PrintStream ps,String toR,String fromR) throws FileNotFoundException,IOException{
	out = ps;
	...
	try{
	    ...
	    // _toR.write(b);
	    // _toR.flush();
	    CodedOutputStream cdo = CodedOutputStream.newInstance(_toR);
	...
	String temp_out_file = r.getRexpValue(3).getStringValue(i).getStrval(); //this is same filename as
	// as this rexp is written into
	CodedOutputStream cdo = CodedOutputStream.newInstance(new FileOutputStream(temp_out_file), 2*1024*1024);
	int counter=0;
	boolean endd=false;
	...
	Richmond r = new Richmond(out,args[0],args[1]);
	try{
	    if(args.length > 0 and args[0].equals("sequenceasbinary")){
		REXP rb = REXP.newBuilder().mergeFrom(new FileInputStream(args[0])).build();
		r.sequenceasbinary(rb);
		System.exit(0);
	    }
	    r.startme();
	}catch(Exception e){
