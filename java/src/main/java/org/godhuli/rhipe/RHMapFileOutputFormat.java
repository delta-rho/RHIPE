...
public class RHMapFileOutputFormat extends FileOutputFormat<RHBytesWritable, RHBytesWritable> {
     ...
     */
    public static MapFile.Reader[] getReaders(Path dir, Configuration conf) throws IOException, FileNotFoundException {
    ...
    public static MapFile.Reader[] getReaders(String[] ps, Configuration conf) throws IOException, FileNotFoundException {
        FileSystem fs = FileSystem.get(conf);
     ...
     */
    public static RHBytesWritable getEntry(MapFile.Reader[] readers, Partitioner<RHBytesWritable, RHBytesWritable> partitioner, RHBytesWritable key, RHBytesWritable value) throws IOException {
        int part = partitioner.getPartition(key, value, readers.length);
        return (RHBytesWritable) readers[part].get(key, value);
    }

    public static RHBytesWritable getEntry(MapFile.Reader[] readers, RHBytesWritable key, RHBytesWritable value) throws IOException {
        int part = hp.getPartition(key, value, readers.length);
        return (RHBytesWritable) readers[part].get(key, value);
    }

    public static MapFile.Reader getPartForKey(MapFile.Reader[] readers, RHBytesWritable key, RHBytesWritable value) throws IOException {
        int part = hp.getPartition(key, value, readers.length);
        return readers[part];
        // return (RHBytesWritable)readers[part].get(key, value);
    }
